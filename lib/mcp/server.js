...
server.tool(
  ...
  async ({ cidade, data_inicio, data_fim }) => {
    try {
      ...
      const data = await consultaClima(cidade, baseUrl, data_inicio, data_fim);
        ...
        content: [
          {
            ...
            text:
              data.mensagem ||
              `O clima médio para a cidade de ${cidade} de ${data_inicio || "o início"} a ${data_fim || "hoje"} foi retornado com sucesso.`,
          },
        ],
        data: data.data,
      };
